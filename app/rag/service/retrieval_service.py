...
def retrieve_data(query: QueryRequestSchema, tenant_id: UUID):
    normalized_query_content = normalize_text(query.content)
    normalized_query = QuerySchema(
        content=normalized_query_content, tenant_id=tenant_id
    )
    ...
    retrieved_chunks = retrieve_chunks(query_embedding, COLLECTION_NAME, TOP_K,tenant_id)
...
def attach_query_id_to_retrieved_chunks(chunks, query_id,tenant_id):
    rows = []
