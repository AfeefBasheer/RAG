...
def retrieve_data(query: QueryRequestSchema, user_id: UUID, tenant_id: UUID):
    ...
    retrieved_embeddings = retrieve_embeddings(
        query_embedding, COLLECTION_NAME, query.document_ids, TOP_K, user_id, tenant_id
    )
    ...
    if not retrieved_chunks_with_query_id:
        return RetrievalResponse(query=normalized_query_content, context=[])
...
def attach_query_id_to_retrieved_chunks(
    embeddings, chunks, query_id, user_id, tenant_id
):
    rows = []
