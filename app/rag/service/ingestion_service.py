...
def ingest_document(document_id: UUID,tenant_id:UUID):
    document = get_document_by_document_id(document_id,tenant_id)
    if not document.document_id == document_id:
        raise "Document Doesn't Exist"

    chunk_response = get_chunks_by_document_id(document_id,tenant_id)
    if not chunk_response:
        raw_chunks = chunk_data_by_chars(
            document.content, CHUNK_SIZE_v1, OVERLAP_SIZE_v1
        )
        chunk_response = insert_chunks(document_id, raw_chunks,tenant_id)
        if chunk_response:
            mark_document_chunked(document_id,tenant_id)

    chunks = extract_chunk_content(chunk_response)
    embedded_data = check_embeddings(COLLECTION_NAME, document_id, len(chunks),tenant_id)
    if not embedded_data:
        embeddings = embed_the_chunks(chunks, TIMEOUT)
        embed_response = insert_embeddings(COLLECTION_NAME, chunk_response, embeddings,tenant_id)
        if embed_response:
            mark_document_embedded(document_id,tenant_id)

    return get_document_by_document_id(document_id,tenant_id)
